...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   /** The optional poke type, species based. */
   ...
   *@param type2 The optional poke type for species.
   ...
   **/
   public Pokemon(String species, String name, 
       ...
       int baseDefensePower, int baseStaminaPower) throws PokemonException {
      ...
      this.type2 = type2;
      if (name.equals("")) {
         throw new PokemonException("Name cannot be null");
      }
   ...
   */
   ...
   /**
    * Compares two Pokemon objects are greater than, less than, or equal to
    * each other.
    *@param poke is the object being compared to this.pokemon
    *@return positive if this > poke, negative if this < poke
    * , and 0 if they are both equal
    */
   public int compareTo(Pokemon poke) {
      int compare;
      compare = this.getNumber() - poke.getNumber();
      if (compare != 0) {
         return compare;
      }
      if (!(this.getName().equals(poke.getName()))) {
         compare = this.getName().compareTo(poke.getName());
         return compare;
      }
      compare = this.getHP() - poke.getHP();
      if (compare != 0) {
         return compare;
      }
      compare = this.getCP() - poke.getCP();
      if (compare != 0) {
         return compare;
      }
      if (!(this.getFastAttack().equals(poke.getFastAttack()))) {
         compare = this.getFastAttack().compareTo(poke.getFastAttack());
         return compare;
      }
      if (!(this.getSpecialAttack().equals(poke.getSpecialAttack()))) {
         compare = this.getSpecialAttack().compareTo(poke.getSpecialAttack());
         return compare;
      }
      else {
         return 0;
  ...
  * @param  poke is being compared to this.
  * @return true if equal, false if unequal
  */
   public Boolean equals(Pokemon poke) {
   
      return this.compareTo(poke) == 0;
   }
     /**
  ...
  * @throws PokemonException string
  */
   ...
   */   
   ...
   /** 
   * Gets the pokeary type.
   * @return String representing this  species' pokeary type.
   * empty String if no pokeary type
   */   
